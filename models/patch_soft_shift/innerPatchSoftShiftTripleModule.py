...
class InnerPatchSoftShiftTripleModule(nn.Module):
    def forward(self, input, stride, triple_w, mask, mask_thred, shift_sz, show_flow):
        ...
        latter_all_windows = latter_all_pad.unfold(2, self.shift_sz, self.stride).unfold(3, self.shift_sz, self.stride)
        latter_all_windows = latter_all_windows.contiguous().view(self.bz, -1, self.c//2, self.shift_sz, self.shift_sz)
        ...
        m = m_pad.unfold(2, self.shift_sz, self.stride).unfold(3, self.shift_sz, self.stride)
        m = m.contiguous().view(1, -1, self.shift_sz, self.shift_sz)
        ...
        torch.set_printoptions(threshold=32000*200)
        # mm: the masked reigon is filled with 0, nonmasked region is filled with 1.
        mm = m.le(self.mask_thred/(1.*self.shift_sz**2)).float() # 1*(32*32)*1*1
        ...
        for idx in range(self.bz):
            ...
            y_i = F.conv2d(former, latter_win_normed, stride=1, padding=self.shift_sz//2)

            # TODO:Add fuse code latter
