...
class EasyUnetGenerator2(nn.Module):
    def __init__(self, input_nc, output_nc, innerCos_list, shift_list, mask_global, opt, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_spectral_norm=False):
        super(EasyUnetGenerator2, self).__init__()
        ...
        self.d6_dc = spectral_norm(nn.ConvTranspose2d(ngf*4*3, ngf*2, kernel_size=6, stride=2, padding=2), use_spectral_norm)
        self.d6_norm = norm_layer(ngf*2)

        self.d7_dc = spectral_norm(nn.ConvTranspose2d(ngf*2*2, ngf, kernel_size=6, stride=2, padding=2), use_spectral_norm)
        self.d7_norm = norm_layer(ngf)

        self.d8_dc = spectral_norm(nn.ConvTranspose2d(ngf*2, output_nc, kernel_size=8, stride=2, padding=3), use_spectral_norm)
    ...
    def forward(self, input):
        ...
        d6 = self.d6_norm(self.d6_dc(self.shift(self.innerCos(F.relu_(torch.cat([d5, e3], dim=1))))))
...
class EasyUnetGenerator3(nn.Module):
    def __init__(self, input_nc, output_nc, innerCos_list, shift_list, mask_global, opt, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_spectral_norm=False):
        super(EasyUnetGenerator3, self).__init__()
    ...
    def forward(self, input):
        ...
        # Decoder
        d1 = self.d1_norm(self.d1_dc(F.leaky_relu_(e8, negative_slope=0.2)))
        d2 = self.d2_norm(self.d2_dc(F.leaky_relu_(torch.cat([d1, e7], dim=1), negative_slope=0.2)))
        d3 = self.d3_norm(self.d3_dc(F.leaky_relu_(torch.cat([d2, e6], dim=1), negative_slope=0.2)))
        d4 = self.d4_norm(self.d4_dc(F.leaky_relu_(torch.cat([d3, e5], dim=1), negative_slope=0.2)))
        d5 = self.d5_norm(self.d5_dc(F.leaky_relu_(torch.cat([d4, e4], dim=1), negative_slope=0.2)))
        d6 = self.d6_norm(self.d6_dc(self.shift(self.innerCos(F.leaky_relu_(torch.cat([d5, e3], dim=1), negative_slope=0.2)))))
        d7 = self.d7_norm(self.d7_dc(F.leaky_relu_(torch.cat([d6, e2], dim=1), negative_slope=0.2)))
        # No norm on the last layer
        d8 = self.d8_dc(F.leaky_relu_(torch.cat([d7, e1], 1), negative_slope=0.2))
...
class EasyUnetGenerator4(nn.Module):
    def __init__(self, input_nc, output_nc, innerCos_list, shift_list, mask_global, opt, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_spectral_norm=False):
        super(EasyUnetGenerator4, self).__init__()
        ...
        self.d3_dc = spectral_norm(nn.ConvTranspose2d(ngf*8, ngf*8, kernel_size=6, stride=2, padding=2), use_spectral_norm)
        self.d3_cc = spectral_norm(nn.Conv2d(ngf*8*2, ngf*8, kernel_size=3, stride=1, padding=1), use_spectral_norm)
        self.d3_norm = norm_layer(ngf*8)

        self.d4_dc = spectral_norm(nn.ConvTranspose2d(ngf*8, ngf*4, kernel_size=6, stride=2, padding=2), use_spectral_norm)
        # shift before this layer
        ...
        self.d7_dc = spectral_norm(nn.ConvTranspose2d(ngf, output_nc, kernel_size=6, stride=2, padding=2), use_spectral_norm)
...
class EasyUnetGenerator5(nn.Module):
    def __init__(self, input_nc, output_nc, innerCos_list, shift_list, mask_global, opt, ngf=64,
                 norm_layer=nn.BatchNorm2d, use_spectral_norm=False):
        super(EasyUnetGenerator5, self).__init__()
        ...
        self.d2_dc = spectral_norm(nn.ConvTranspose2d(ngf*8 , ngf*8, kernel_size=4, stride=2, padding=1), use_spectral_norm)
        self.d2_cc = spectral_norm(nn.Conv2d(ngf*8*2, ngf*8, kernel_size=3, stride=1, padding=1), use_spectral_norm)
        self.d2_norm = norm_layer(ngf*8)
