...
from .InnerCosSingleFunction import InnerCosSingleFunction

class InnerCosSingle(nn.Module):
    def __init__(self, crit='MSE', strength=1, skip=0, layer_to_last=3):
        super(InnerCosSingle, self).__init__()
        self.crit = crit
    ...
    def forward(self, in_data):
        ...
        if not self.skip:
            ...
            self.output = InnerCosSingleFunction.apply(in_data, self.criterion, self.strength, self.target, self.mask)
            # directly using in_data as the target.
            self.target = in_data.detach() # the latter part
        else:
            self.output = in_data
        return self.output
