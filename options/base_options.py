...
class BaseOptions():
    ...
    def initialize(self):
        self.parser.add_argument('--dataroot', default='./datasets/Paris_StreetView_Dataset', help='path to images (should have subfolders trainA, trainB, valA, valB, etc)')
        self.parser.add_argument('--batchSize', type=int, default=1, help='input batch size')
        self.parser.add_argument('--loadSize', type=int, default=350, help='scale images to this size')
        ...
        self.parser.add_argument('--which_model_netG', type=str, default='unet_shift_triple_MostAdvCos', help='selects model to use for netG')  # use unet_swap
        self.parser.add_argument('--triple_weight', type=float, default=1, help='The weight on the gradient of skip connections from the gradient of swapped')
        self.parser.add_argument('--name', type=str, default='256C_TC_TS_g10_IN_cos32_gan0.2', help='name of the experiment. It decides where to store samples and models')
        ...
        self.parser.add_argument('--use_dropout', action='store_true', help='use dropout for the generator') #it means if using '--use_dropout then this option is true.
        ...
        self.parser.add_argument('--threshold', type=float, default=5/16.0, help='making binary mask')
        self.parser.add_argument('--stride', type=int, default=1, help='should be dense, 1 is a good option.')
        self.parser.add_argument('--shift_sz', type=int, default=1, help='when it is 3, the next opt `mask_thred` should not be 1')
        self.parser.add_argument('--mask_thred', type=int, default=1, help='number to decide whether a patch is masked')
    ...
    def parse(self):
        ...
        # set gpu ids (Should set default gpu here!)  ?? Not support multi-GPU since this changes?
        if len(self.opt.gpu_ids) > 0:
            torch.cuda.set_device(self.opt.gpu_ids[0])
        
        args = vars(self.opt)  # it makes self.opt a dict
